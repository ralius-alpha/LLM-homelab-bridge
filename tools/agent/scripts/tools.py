...
SKILL_USAGE_NOTES = {
    "search_web": (
        ...
        "一般的な設計相談・雑談。"
        "[IMPORTANT] 返ってくるのは検索結果の一覧（タイトル・URL・短い抜粋）だけで、"
        "知りたい中身そのものではない。実際の値や本文が必要な依頼では、"
        "続けてfetch_urlで本文を取得してから答えること。"
        "リンクを並べて「こちらでご確認ください」で終わらせるのは、"
        "ユーザーに調べさせているのと同じで誤り。"
        "また、検索結果の上位にはポータルのトップページが並びやすい。"
        "トップページは一般的な見出ししか載っていないことが多いので、"
        "知りたい対象そのもののページを選ぶこと。"
    ),
    "fetch_url": (
