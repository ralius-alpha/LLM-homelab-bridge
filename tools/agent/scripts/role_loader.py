...
def _inject_preflight_check(prompt):
    ...
    block = [
        ...
        "「〜でよろしいですか？」を毎回付けるのも不要。",
        "[IMPORTANT] 依頼の中に対象と、してほしいことが具体的に書かれているなら、"
        "それは【揃っている】。聞き返さず、そのまま実行するか適切な役に引き継ぐこと。"
        "例えば対象・変更箇所・変更後の値がすべて書かれている依頼を、"
        "「本当に変更したいのか」「やり方を知りたいだけではないか」と"
        "問い直すのは、確認ではなく作業の先延ばしにあたる。"
        "ユーザーが依頼として書いたことは、依頼として受け取ること。",
